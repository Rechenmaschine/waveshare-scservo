...
pub use series::{ServoMode, ServoSeries, ServoTelemetry, SCSCL, SMS_STS};
pub use types::{Instruction, ScsPositionMove, ScsServoState, ScsStatus};
...
/// Backward compatible alias for SCSCL bus.
#[cfg(feature = "scscl")]
pub type SCLBus<I> = ScsclBus<I>;

/// Resolution of the servo in steps (SCSCL: 0-1023, SMS_STS: 0-4095)
pub const RESOLUTION_STEPS: u16 = 1024;
/// Maximum effective angle in degrees (SCSCL)
...
/// Maximum position value (steps) for SCSCL
...
/// Encode signed speed for wheel mode (bit 15 = sign).
#[cfg(feature = "sms_sts")]
#[allow(clippy::cast_sign_loss)]
pub(crate) fn encode_signed_speed(speed: i16) -> u16 {
    if speed < 0 {
        ((-speed) as u16) | BIT_15_SIGN
    } else {
        speed as u16
    }
}
...
mod tests {
    ...
    #[cfg(feature = "sms_sts")]
    #[test]
    fn test_encode_signed_speed() {
        assert_eq!(encode_signed_speed(100), 100);
        assert_eq!(encode_signed_speed(-100), 0x8064);
        assert_eq!(encode_signed_speed(0), 0);
    }

    #[test]
    fn test_degrees_conversion() {
        let steps = degrees_to_steps(110.0);
        let degrees = steps_to_degrees(steps);
